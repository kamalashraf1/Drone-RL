...
class ArduPilotContinuousTrainer2(gym.Env):
  ...
  def step(self, action):
    ...
    self.logger_2.write(str(self.sensors['gps']['lat']) + ", " + str(self.sensors['gps']['lon']) + "\n")
    ...
    # normalized_array = np.array([(self.modified_data['gps']['lat'] - self.home.lat)*1000, (self.modified_data['gps']['lon'] - self.home.lon)*1000, (self.sensors['gps']['lon'] - self.home.lon)*1000])
    normalized_array = np.array([(self.modified_data['gps']['lat'] - self.home.lat)*1000, (self.modified_data['gps']['lon'] - self.home.lon)*100000, (self.sensors['gps']['lon'] - self.home.lon)*0])
  ...
  def reset(self):
    ...
    self.netHandler.relay_servos_messages()
    # normalized_array = np.array([(self.modified_data['gps']['lat'] - self.home.lat)*1000, (self.modified_data['gps']['lon'] - self.home.lon)*1000, self.modified_data['gps']['alt']])
    # normalized_array = np.array([(self.modified_data['gps']['lat'] - self.home.lat)*1000, (self.modified_data['gps']['lon'] - self.home.lon)*1000, (self.sensors['gps']['lon'] - self.home.lon)*1000])
    normalized_array = np.array([(self.modified_data['gps']['lat'] - self.home.lat)*1000, (self.modified_data['gps']['lon'] - self.home.lon)*100000, (self.sensors['gps']['lon'] - self.home.lon)*100000])
  ...
  def situation_done(self):
    ...
    if target_dest < 10 or adver < 10 or lat_diff < 0.00002 or lat_diff_adv < 0.00002:
