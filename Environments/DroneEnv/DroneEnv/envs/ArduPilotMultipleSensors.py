...
class MiddleWare():
    ...
    def __init__(self,other_ip,lim):
        ...
        self.velocity_bound = 0.001
        self.accel_bound = 0.0001
    ...
    def modify_sensor_data(self,sensors,modifiers,modified_data):
        ...
        modifiers_local = modifiers[0]
        sensors['gps']['lon'] = sensors['gps']['lon'] + np.clip(modifiers_local[0],-(self.lim),self.lim)
        # sensors['gps']['lat'] = sensors['gps']['lat'] + np.clip(modifiers_local[1],-(self.lim),self.lim)
        for i in range(3):
          sensors['imu']['angular_velocity'][i] =  sensors['imu']['angular_velocity'][i] + np.clip(modifiers_local[2+i],-(self.velocity_bound),self.velocity_bound)
          sensors['velocity']['world_linear_velocity'][i] =  sensors['velocity']['world_linear_velocity'][i] + np.clip(modifiers_local[5+i],-(self.velocity_bound),self.velocity_bound)
        return sensors

        # Use following for extending the amount of data
        """
        sensors['imu']['linear_acceleration'] = sensors['imu']['linear_acceleration'] + (np.clip(modifiers_local[2:5],-(self.lim),self.lim)).tolist()
    ...
    def convert_to_numpy(self,sensors):
        ...
        sensor_arr = sensor_arr + sensors['imu']['angular_velocity']
...
class ArduPilotMultipleSensors(gym.Env):
  metadata = {'render.modes': ['human']}

  def __init__(self,other_ip,lim,dest,adverse,reward_fn):
  ...
  def step(self, action):
    ...
    self.actual_state = self.netHandler.convert_to_numpy(self.sensors)
    self.costs = self.calculate_costs()
    self.done = self.situation_done()
    self.times = self.times+1 
    # normalized_array = [1000*(self.state[0] - self.home.lat), 1000*(self.state[1] - self.home.lon)]
    # normalized_array += (self.state[3:]).tolist()
    normalized_array = [10*(self.state[0] - self.home.lat), 100*(self.state[1] - self.home.lon)]
    normalized_array += (1000*self.state[3:6]).tolist() + (100*self.state[6:8]).tolist() + (self.state[8:9]).tolist() + (1000*self.state[9:11]).tolist() + (self.state[11:]).tolist()
    normalized_array = np.array(normalized_array)
    return normalized_array ,self.costs, self.done, {}
  
  def calculate_costs(self):
    longitude_diff = abs(self.adverse_location.lon - self.actual_state[1])
    if self.reward_fn == 0:
      cost = -longitude_diff
      if (self.vehicle.error_tracker - self.old_error_num) > 0:
        print("COST : error updated")
        cost -= 0.001 * (self.vehicle.error_tracker - self.old_error_num)
        self.old_error_num = self.vehicle.error_tracker
      if self.vehicle.system_status.state == "CRITICAL":
        print("COST: critical state")
        cost -= 0.001 *1
      if self.vehicle.system_status.state == "EMERGENCY":
        cost -= 0.001 *2
        print("COST: emergency state")
      if not self.vehicle.ekf_ok:
        cost = cost - 0.001 * 3
        print("COST: ekf not okay")
      return cost
  ...
  def reset(self):
    ...
    normalized_array += (self.state[3:]).tolist()
    self.old_error_num = self.vehicle.error_tracker
    self.error_num_init = self.vehicle.error_tracker
    self.actual_state = self.netHandler.convert_to_numpy(self.sensors)
    normalized_array = [10*(self.state[0] - self.home.lat), 100*(self.state[1] - self.home.lon)]
    normalized_array += (1000*self.state[3:6]).tolist() + (100*self.state[6:8]).tolist() + (self.state[8:9]).tolist() + (1000*self.state[9:11]).tolist() + (self.state[11:]).tolist()
    normalized_array = np.array(normalized_array)
  ...
  def not_at_home(self):
    dest = self.get_distance_metres(self.home,self.actual_state[0],self.actual_state[1])
    if self.start :
      self.start = False
      return dest > 15
      return False
  ...
  def print_stats(self):
    print("lat : " + str(self.actual_state[0]) + " lon : " + str(self.actual_state[1]))
    print("Critical and emergency counts : " + str(self.old_error_num - self.error_num_init))
    
  def situation_done(self):
    target_dest = self.get_distance_metres(self.dest,self.actual_state[0],self.actual_state[1])
    adver = self.get_distance_metres(self.adverse_location,self.actual_state[0],self.actual_state[1])
    lat_diff = abs(self.actual_state[0] - self.dest.lat)
    lat_diff_adv = abs(self.modified_data['gps']['lat'] - self.dest.lat)
    if self.old_error_num > 40:
      print("Very high error")
      return True
    return target_dest < 10 or adver < 10 or lat_diff < 0.00002 or lat_diff_adv < 0.00002 or self.vehicle.location.global_relative_frame.alt < 20 
